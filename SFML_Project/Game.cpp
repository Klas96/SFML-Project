...
Game::Game(){
	...
	//Gör ny player array varför redan nu varför inte senare
...
//game update run each iteration
void Game::Update(float dt){

	//Skaffa texter borde kanske inte köras varje update??
	playerText.setString("Players\n" + GetAllPlayerInfo());
	roundText.setString("Round\t" + std::to_string(roundCount));
	...
	if (RoundEnded() == false){
		...
		if (powerTime >= 3.0f && playerHasPowerup == false && powerupExists == false){
